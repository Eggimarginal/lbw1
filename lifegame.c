/* Содержимое этого файла менять не нужно!.
   Весь ваш код должен быть в main.c   */
...
/* символы представляющие состояния ячеек */
#define CHAR_ALIVE '*'
#define CHAR_DEAD ' '

/* текущие состояния ячеек */
static int world[WORLDWIDTH][WORLDHEIGHT];

/* состояния ячеек следующего поколянеия */
static int nextstates[WORLDWIDTH][WORLDHEIGHT];

/* инициализирует мир заранее заданным шаблоном, 
   сбрасывает все ячейки следующего поколения в DEAD */
